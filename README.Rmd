...
d3wordcloud is a wrapper for the [Word Cloud Layout](http://www.jasondavies.com/wordcloud) by [Jason Davies](http://www.jasondavies.com) based on [htmlwidgets](https://github.com/ramnathv/htmlwidgets).

## How it works?!

The main function `d3wordcloud` needs only `words` and `freqs`. Just like the old good wordcloud package. See demo [here](http://r-shiny-apps.jkunst.com/d3wordcloud/). 
...
There are parameter for make your word cloud like you like/want:
  ...
  - `colors`: The color for wordcloud, if the length of words, and colors are the
  ...
  - `size.scale`: The scale to use for scale the words sizes (`freqs`). Options are
  `linear`, `sqrt` and `log`. Default value is `linear`.
  - `color.scale`: The scale to use for scale the colors according to sizes (`freqs`). Options are
  `linear`, `sqrt` and `log`. Default value is `linear`.
...
You can see this parameter in action here http://rpubs.com/jbkunst/100416.

## Demo

Check here https://jbkunst.shinyapps.io/d3wordcloud. 

Sometimes I exceed the hours on the basic plan in shiniapps.io. You can always can view here: http://rpubs.com/jbkunst/100416.
